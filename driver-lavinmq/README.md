...
In order to create the local artifacts necessary to run the LavinMQ benchmarks in AWS, you'll need to have [Maven](https://maven.apache.org/install.html) installed. Once Maven's installed, you can create the necessary artifacts with a single Maven command:
...
Once those conditions are in place, you'll need to create an SSH public and private key at `~/.ssh/lavinmq_aws` (private) and `~/.ssh/lavinmq_aws.pub` (public), respectively.

```bash
$ ssh-keygen -f ~/.ssh/lavinmq_aws
```
...
|:--------------------|:------------------------------------------------------------|:------|
| LavinMQ instances   | The VMs on which LavinMQ brokers will run                   | 3     |
...
> If you're using an SSH private key path different from `~/.ssh/lavinmq_aws`, you can specify that path using the `--private-key` flag, for example `--private-key=~/.ssh/my_key`.
...
```bash
$ ssh -i ~/.ssh/lavinmq_aws ec2-user@$(terraform output client_ssh_host)
```
...
```bash
$ cd /opt/benchmark
$ sudo bin/benchmark --drivers driver-lavinmq/lavinmq-classic.yaml workloads/*.yaml
```

You can also run specific workloads in the `workloads` folder. Here's an example:

```bash
$ sudo bin/benchmark --drivers driver-lavinmq/lavinmq-classic.yaml workloads/1-topic-1-partitions-1kb.yaml
```
...
The LavinMQ UI is available and the HTTP endpoint is **publicly** exposed on all LavinMQ instances
...
the `admin` account configured in the [Terraform](deploy/provision-lavinmq-aws.tf) file to log in.

### Prometheus

Prometheus is installed on a standalone instance, along with
...
`http://${prometheus_host}:3000`. Credentials are `admin`/`admin`.
