...
import com.lavinmq.client.Address;
import com.lavinmq.client.AlreadyClosedException;
import com.lavinmq.client.BuiltinExchangeType;
import com.lavinmq.client.Channel;
import com.lavinmq.client.Connection;
import com.lavinmq.client.ConnectionFactory;
...
public class LavinMqBenchmarkDriver implements BenchmarkDriver {

    private LavinMqConfig config;
    ...
    public void initialize(File configurationFile, StatsLogger statsLogger) throws IOException {
        config = mapper.readValue(configurationFile, LavinMqConfig.class);
    }
    ...
    public CompletableFuture<Void> createTopic(String topic, int partitions) {
        if (partitions != 1) {
            throw new IllegalArgumentException("Cannot create topic with partitions in LavinMQ");
        }
                ...
                .execute(
                        () -> {
                            try {
                                ...
                                future.complete(
                                        new LavinMqBenchmarkProducer(channel, exchange, config.messagePersistence));
                            } catch (Exception e) {
                ...
                .execute(
                        () -> {
                            try {
                                ...
                                future.complete(
                                        new LavinMqBenchmarkConsumer(channel, queueName, consumerCallback));
                            } catch (IOException e) {
                                future.completeExceptionally(e);
                            }
                        });
    ...
    private Connection getOrCreateConnection(String primaryBrokerUri) {
        return connections.computeIfAbsent(
                primaryBrokerUri,
                p -> {
                    // LavinMQ will pick the first available address from the list. Future reconnection
    ...
    private static final Logger log = LoggerFactory.getLogger(LavinMqBenchmarkDriver.class);
